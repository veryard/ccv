...
export async function getLatestVersion(octokit: Octokit, owner: string, repo: string, prefix: string): Promise<string> {
  ...
  for (const tag of list) {
    if (prefix) {
      tag.name = tag.name.startsWith(prefix) ? tag.name.replace(prefix, '') : tag.name;
    }
